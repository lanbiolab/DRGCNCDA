...
class MessageGat(Model):
    ...
    def get_vertex_features(self, senders=True, mode='train'):  # 获取顶点特征
        if senders:
    ...
    def compute_vertex_embeddings(self, mode='train'):
        if self.vertex_embedding_function[mode] is None:
            ...
            if self.onehot_input:
                self.vertex_embedding_function[mode] = self.inference(sender_features+receiver_features,
                                                                             self_loop_messages,
                                                                             tf.range(self.entity_count),
                                                                             mode=mode)
            else:
                self.vertex_embedding_function[mode] = self.inference(sender_features,
                                                                             self_loop_messages,
                                                                             self.next_component.get_all_codes(
                                                                                 mode=mode)[0],
                                                                             mode=mode)
