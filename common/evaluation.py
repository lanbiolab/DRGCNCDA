...
class AUCSummary():
    ...
    def __init__(self, predictions):
        self.results['Filtered'][self.auc_string()] = np.mean(predictions)
...
class AUCScore():
    ...
    def get_summary(self):
        return AUCSummary(self.predictions)
...
class Scorer():
    ...
    def __init__(self, settings, file_index):
    ...
    def compute_auc(self, triples, verbose = False):
        score = AUCScore()
        if verbose:
        ...
        print(scores)
        np.savetxt("score_matrix_5fold"+"28.txt", scores, delimiter = ",")
        # positives = score_vector[::2]
        # negatives = score_vector[1::2]
        positives = []
        negatives = []
        for i in range(0, int(len(score_vector)/2)):
            positives.append(score_vector[i])
            negatives.append(score_vector[i+1])
    ...
    def compute_scores(self, triples, verbose=False):
        if self.settings['Metric'] == 'MRR':
            ...
            return self.compute_accuracy_scores(triples, verbose=verbose)#triples代表的是测试集
        elif self.settings['Metric'] == 'AUC':
            return self.compute_auc(triples, verbose=verbose)
